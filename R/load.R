...
onerm_epley <- function(load, repitions) {
  ifelse(repititions >= 1, load * (1 + (repetitions/30)), warning("Repititions must be >= 1"))
}
...
onerm_brzycki <- function(load, repitions) {
  ifelse(repititions >= 1, load / (1.0278 - 0.0278 * repetitions), warning("Repititions must be >= 1"))
}
...
onerm_lombardi <- function(load, repitions) {
  ifelse(repititions >= 1, load * repetitions^0.1, warning("Repititions must be >= 1"))
}
...
onerm_oconner <- function(load, repitions) {
  ifelse(repititions >= 1, load * (1 + (repetitions/40)), warning("Repititions must be >= 1"))
}
...
onerm_wathen <- function(load, repitions) {
  ifelse(repititions >= 1, (100 * load) / (48.8 + 53.8 * exp(-0.075 * repetitions)), warning("Repititions must be >= 1"))
}
